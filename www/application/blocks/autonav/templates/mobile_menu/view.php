...
if (count($navItems) > 0) {
    ...
    foreach ($navItems as $ni) {
        $showSubmenu = $ni->isCurrent ? 'show-submenu' : '';
        ...
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '">'  . $niPageIconHtml . 
            h($ni->name) . '</a>' . (($ni->hasSubmenu) ? '<span class="toggle-submenu"><i class="fa fa-angle-down"></i></span>' : '');

        if ($ni->hasSubmenu) {
            echo '<ul class="' . $showSubmenu . '">'; //opens a dropdown sub-menu
        } else {
